...
First day of Pet Them All. No gameplay yet: today we laid the groundwork.

## The project rules

We wrote two convention files for Claude, which helps us develop the game.

For **gameplay**, every feature gets a test before we move on to the next one. At this stage we only work on mechanics, with placeholder geometry: cubes and spheres, no final assets.

For **art in Blender**:

- previews rendered with Eevee;
- at most 2,000 triangles for props and 20,000 for main characters;
- final assets exported as GLB;
- source and license of every asset recorded in `CREDITS.md`.

## A devlog that doesn't forget

We added an automatic check: when Claude finishes working, it looks for commits from today that have no devlog post. If the post is missing, it asks for one. This is the first.
